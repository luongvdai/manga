$(document).ready(async function () {
  ...
  let comicId = parseInt(params.get("ComicId")) || 0;
  let TotalPage;
  let loginUrl = "http://localhost:5206/Home/Login";
 /* $("#unfollow").addClass("d-none");*/
  //load các loại truyện trên nav
    loadCategory();
    //Load các button Follow
    loadFollow()
    function loadFollow() {
        $("#loadFollow").html("");
        $.ajax({
            url: 'http://localhost:5278/api/Users/' + userId,
            type: 'GET',
            contentType: 'application/json',
            success: function (data, status, xhr) {
                var comic = data["comicsNavigation"]?.find(value => value["comicId"] === comicId);
                if (comic) {
                    $("#loadFollow").append($("<button>")
                        .attr("id", "unfollow")
                        .addClass("btn btn-outline-danger text-wrap").html("<i class=\"ti ti-x\"></i>Bỏ theo dõi"));
                } else {
                    $("#loadFollow").append($("<button>")
                        .attr("id", "follow")
                        .addClass("btn btn-success").html("<i class=\"ti ti-bookmark-plus\"></i>Theo dõi"));
                }
                console.log(data);
            },
            error: (e) => {
  ...
  function handleClick(button, url, message) {
    ...
    handleClick(
        "#follow",
        "http://localhost:5278/api/Users/follow/" + userId + "/" + comicId,
      "Theo dõi thành công!"
    );
  });
    ...
    handleClick(
      "#unfollow",
        "http://localhost:5278/api/Users/unfollow/" + userId + "/" + comicId,
